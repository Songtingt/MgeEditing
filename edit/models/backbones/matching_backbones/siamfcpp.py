...
def get_xy_ctr_np(score_size, score_offset, total_stride): #37,255.5 8
    ...
    y_list = y_list.repeat(fm_width, axis=3) #1,1,37,37 每一行都是 0000 1111
    x_list = np.linspace(0., fm_width - 1., fm_width).reshape(1, 1, 1, fm_width)
    x_list = x_list.repeat(fm_height, axis=2)
    xy_list = score_offset + np.concatenate((y_list, x_list), 1) * total_stride #把feature map上每个点的坐标映射到原图上  作为中心
...
class SIAMFCPP(M.Module):
    ...
    def __init__(self, in_cha,
        ...
        self.score_offset = (x_size - 1 - (self.score_size - 1) * self.total_stride) / 2  # 255.5
    ...
    def _init_cls_convs(self):
        ...
        self.cls_convs = []
        for i in range(self.stacked_convs-1): #3个卷积层
    ...
    def head(self, c_out, r_out):
        c_out = self.cls_convs(c_out) #对互相关结果再做个分类卷积
        r_out = self.reg_convs(r_out)

        # classification score
        cls_score = self.conv_cls(c_out)  # [B,1,37,37] 最终的分类结果
    ...
    def forward(self, sar, optical):
        ...
        c_z_k = self.c_z_k(feat1)  # 100, 64 分别使用分类头和回归头
    ...
    def get_cls_reg_ctr_targets(self, points, gt_bboxes, bbox_scale = 0.15):
        ...
        gap = (gt_bboxes[:, 2, ...] - gt_bboxes[:, 0, ...]) * (1-bbox_scale) / 2  #求出bbox的边长
        ...
        up_left = points - gt_bboxes[:, 0:2, ...]  # (B, 2, 37, 37) score map每个点和左上角点的差
    ...
    def loss(self,
        ...
        # reg
        bbox_preds = F.concat([bbox_preds, self.z_size -1 - bbox_preds], axis = 1)  # [B,4,37,37] 预测出来的只有两个值
