...
@PIPELINES.register_module()
class RandomShelter(object):
    def __init__(self, keys, shelter_ratio=0.5):
        self.keys = keys
        self.shelter_ratio = shelter_ratio

    def __call__(self, results):
        do_shelter = np.random.random() < self.change_ratio
        if do_shelter:

        results['Shelter']=do_shelter
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += (
            f'(keys={self.keys}, shelter_ratio={self.shelter_ratio})')
        return repr_str
